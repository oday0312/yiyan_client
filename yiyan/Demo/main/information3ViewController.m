...
- (id)initWithTitle:(NSString *)title withRevealBlock:(RevealBlock)revealBlock {
    if (self = [super initWithNibName:@"information3ViewController" bundle:nil]) {
		self.title = title;
...
- (void)viewDidLoad
{
    ...
    self.inputTextField.text = @"请输入要查询的IP地址";
...
-(void)startHttp:(NSString*)inputString
{
    if ([inputString isEqualToString:@"a"]) {
        inputString =@"106.3.83.132";
    }
    
    /////http://www.ip.cn/getip.php?action=queryip&ip_url=106.3.83.132&from=web
    NSString* urlstring = [@"" stringByAppendingFormat:@"http://www.ip.cn/getip.php?action=queryip&ip_url=%@&from=web",inputString];
...
-(void)getPagesInformation:(NSString*)inputstring
{
    ...
    self.resultLabel.text = (NSMutableString*)@"";
